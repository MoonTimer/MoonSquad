...
public abstract class EntityLivingBase extends Entity {
    ...
    /**
     * SpeedMine taken in the last hit. Mobs are resistant to damage less than this for a short time after taking damage.
     */
    ...
    protected void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos) {
        ...
        if (!this.worldObj.isRemote && this.fallDistance > 3.0F && onGroundIn) {
            ...
            if (block.getMaterial() != Material.air) {
                ...
                int i = (int) (150.0D * d0);
                ((WorldServer) this.worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ, i, 0.0D, 0.0D, 0.0D, 0.15F, Block.getStateId(iblockstate));
            }
        }
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if (flag) {
                    ...
                    if (entity != null) {
                        ...
                        this.attackedAtYaw = (float) (MathHelper.atan2(d0, d1) * 180.0D / (double) (float) Math.PI - (double) this.rotationYaw);
                        this.knockBack(d1, d0);
                    } else {
    ...
    protected void collideWithNearbyEntities() {
        List<Entity> list = this.worldObj.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.2F, 0.0D, 0.2F), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBePushed));
