...
public class EntityArrow extends Entity implements IProjectile {
    ...
    public EntityArrow(World worldIn, EntityLivingBase shooter, EntityLivingBase target, float velocity, float innacuracy) {
        ...
        if (!(d3 < 1.0E-7D)) {
            ...
            float f1 = (float) (-(MathHelper.atan2(d1, d3) * 180.0D / (double) (float) Math.PI));
            double d4 = d0 / d3;
            double d5 = d2 / d3;
    ...
    public void setThrowableHeading(double x, double y, double z, float velocity, float inaccuracy) {
        ...
        y = y / (double) f;
        z = z / (double) f;
        x = x + this.rand.nextGaussian() * (double) (this.rand.nextBoolean() ? -1 : 1) * (double) 0.0075F * (double) inaccuracy;
        y = y + this.rand.nextGaussian() * (double) (this.rand.nextBoolean() ? -1 : 1) * (double) 0.0075F * (double) inaccuracy;
        z = z + this.rand.nextGaussian() * (double) (this.rand.nextBoolean() ? -1 : 1) * (double) 0.0075F * (double) inaccuracy;
        x = x * (double) velocity;
        y = y * (double) velocity;
        z = z * (double) velocity;
        this.motionX = x;
        ...
        this.prevRotationPitch = this.rotationPitch = (float) (MathHelper.atan2(y, f1) * 180.0D / (double) (float) Math.PI);
    ...
    public void setVelocity(double x, double y, double z) {
        ...
        if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) {
            ...
            this.prevRotationYaw = this.rotationYaw = (float) (MathHelper.atan2(x, z) * 180.0D / (double) (float) Math.PI);
            this.prevRotationPitch = this.rotationPitch = (float) (MathHelper.atan2(y, f) * 180.0D / (double) (float) Math.PI);
    ...
    public void onUpdate() {
        ...
        if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) {
            ...
            this.prevRotationYaw = this.rotationYaw = (float) (MathHelper.atan2(this.motionX, this.motionZ) * 180.0D / (double) (float) Math.PI);
            this.prevRotationPitch = this.rotationPitch = (float) (MathHelper.atan2(this.motionY, f) * 180.0D / (double) (float) Math.PI);
        }
        ...
        if (this.inGround) {
                    ...
                    if (movingobjectposition.entityHit.attackEntityFrom(damagesource, (float) l)) {
                        if (movingobjectposition.entityHit instanceof EntityLivingBase) {
                            ...
                            if (this.knockbackStrength > 0) {
                                ...
                                if (f7 > 0.0F) {
                                    movingobjectposition.entityHit.addVelocity(this.motionX * (double) this.knockbackStrength * (double) 0.6F / (double) f7, 0.1D, this.motionZ * (double) this.knockbackStrength * (double) 0.6F / (double) f7);
                                }
                            }
            ...
            for (this.rotationPitch = (float) (MathHelper.atan2(this.motionY, f3) * 180.0D / (double) (float) Math.PI); this.rotationPitch - this.prevRotationPitch < -180.0F; this.prevRotationPitch -= 360.0F) {
            }
