...
public abstract class GuiSlot {
    ...
    protected void overlayBackground(int startY, int endY) {
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        this.mc.getTextureManager().bindTexture(new ResourceLocation("Moon/menu/OptionsBG.png"));
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldrenderer.pos(this.left, endY, 0.0D).tex(0.0D, (float) endY).color(64, 64, 64, 255).endVertex();
        worldrenderer.pos(this.left + this.width, endY, 0.0D).tex((float) this.width, (float) endY).color(64, 64, 64, 255).endVertex();
        worldrenderer.pos(this.left + this.width, startY, 0.0D).tex((float) this.width, (float) startY).color(64, 64, 64, 255).endVertex();
        worldrenderer.pos(this.left, startY, 0.0D).tex(0.0D, (float) startY).color(64, 64, 64, 255).endVertex();
        tessellator.draw();
    }
