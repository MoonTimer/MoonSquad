...
public class GuiOptions extends GuiScreen implements GuiYesNoCallback {
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiButton(201, width / 2 - 75, height / 6 + 22 - 6, 150, 20, I18n.format("Moon (天気)")));
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.enabled) {
            ...
            if (button.id == 201) {
                try {
                    URI uri = new URI("https://en.wikipedia.org/wiki/Moon");
            ...
            if (button.id == 8675309) {
                try {
                    URI uri = new URI("https://moontimer.me/");
                    Desktop.getDesktop().browse(uri);
                } catch (Exception var3) {
                    var3.printStackTrace();
                }
