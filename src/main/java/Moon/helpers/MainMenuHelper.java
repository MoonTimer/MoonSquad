...
import Moon.Moon;
import Moon.managers.GuiAltManager;
...
public class MainMenuHelper extends GuiScreen {
    ...
    public static void displayScreen() {
        Minecraft.getMinecraft().displayGuiScreen(new LicenseHelper());
    }

    public void drawScreen(int mouseX, int mouseY, float p) {
        if (!Moon.INSTANCE.authorised)
            System.exit(0);
        if (LicenseHelper.license == null)
            System.exit(0);
        Minecraft.getMinecraft().gameSettings.ofFastRender = true;
        this.mc.getTextureManager().bindTexture(new ResourceLocation("Moon/menu/MainMenu.png"));
        ...
        OpenGLHelper.INSTANCE.drawLine(this.fade + 1, height / 2 + 30, this.fade - 20, height / 2, 10, 15597803);
        OpenGLHelper.INSTANCE.drawLine(this.fade + 1, height / 2 - 30, this.fade - 20, height / 2, 10, 15597803);
        if (this.In) {
