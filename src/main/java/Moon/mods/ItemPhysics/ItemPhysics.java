...
public class ItemPhysics {
    ...
    public static void doRender(Entity par1Entity, double x, double y, double z, float par9) {
        rotation = (double) (System.nanoTime() - tick) / 3900000.0D;
        ...
        if (itemstack.getItem() != null) {
            ...
            if (!Double.isNaN(entityitem.posX) && !Double.isNaN(entityitem.posY) && !Double.isNaN(entityitem.posZ) && entityitem.worldObj != null) {
                if (entityitem.onGround) {
                    if (entityitem.rotationPitch != 0.0F && entityitem.rotationPitch != 90.0F && entityitem.rotationPitch != 180.0F && entityitem.rotationPitch != 270.0F) {
                        ...
                        } else if (d2 < d1 && d2 < d0 && d2 <= d3) {
                            if (entityitem.rotationPitch - 180.0F < 0.0F) {
                                entityitem.rotationPitch = (float) ((double) entityitem.rotationPitch + rotation);
                            } else {
                                entityitem.rotationPitch = (float) ((double) entityitem.rotationPitch - rotation);
                            }
                        } else if (d3 < d1 && d3 < d2 && d3 < d0) {
