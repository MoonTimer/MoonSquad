...
import static net.optifine.reflect.Reflector.GuiMainMenu;
...
public class ProxyConfig {
    ...
    public void onGlobalGameLoop(EventTick e) {
        if (proxy != null && proxy.isRunning() && proxy.hasFailed()) {
            ...
            if (flag) {
                Helper.mc.displayGuiScreen(new GuiDisconnected(new GuiMultiplayer(new MainMenuHelper()), "disconnect.closed", new ChatComponentText("[AC-Proxy] Lost Connection to server!")));
            } else if (flag1) {
