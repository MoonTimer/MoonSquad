...
public class Criticals extends Module {
    ...
    private boolean canCrit(Entity target) {
        return this.isEnabled() && mc.thePlayer.onGround && !mc.thePlayer.isOnLadder() && !mc.thePlayer.isInWater() && !mc.thePlayer.isInLava() && mc.thePlayer.ridingEntity == null && target.hurtResistantTime <= this.hurtTime.getValue().intValue();
    }
...
//                if (canCrit(entity))
            ...
            case "Packet":
                if (mc.thePlayer.onGround) {
                    mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + 0.15f, mc.thePlayer.posZ, false));
                    mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, false));
                } else {
                    mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY - 0.01f, mc.thePlayer.posZ, false));
                }
                break;
            case "Jump":
                if (canCrit(entity))
